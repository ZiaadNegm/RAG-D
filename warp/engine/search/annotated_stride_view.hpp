...
struct annotated_stride_view {
    ...
    int64_t *winner_pos_;  // Optional: tracks winning embedding position per doc (nullptr if unused)
};
...
// Overload with winner_pos tensor for M3 winner tracking
std::vector<annotated_stride_view<>> strided_view_with_winners(
        const torch::Tensor capacities, torch::Tensor sizes,
        torch::Tensor pids, torch::Tensor scores, torch::Tensor winner_pos) {
    int32_t *size_ptr = sizes.data_ptr<int32_t>();
    int32_t *pids_ptr = pids.data_ptr<int32_t>();
    float *scores_ptr = scores.data_ptr<float>();
    int64_t *winner_pos_ptr = winner_pos.data_ptr<int64_t>();
    ...
    for (int i = 0; i < num_cells; ++i) {
        ...
        views.push_back(annotated_stride_view<>{
            ...
            .data_ = scores_ptr + begin,
            .counts_ = nullptr,
            .winner_pos_ = winner_pos_ptr + begin
        });
        begin += capacity;
    }
    return views;
