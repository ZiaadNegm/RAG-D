...
// Thread-safe: each cell writes to disjoint slice of output tensors
template<int8_t nbits>
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> parallel_decompress_centroids_with_winners(
    const torch::Tensor begins,
    const torch::Tensor ends,
    const torch::Tensor sizes,
    const torch::Tensor centroid_scores,
  ...
  at::parallel_for(0, nprobe * num_query_tokens, 1, [&](int64_t begin, int64_t end) {
    for (int cell_idx = begin; cell_idx < end; ++cell_idx) {
      const int64_t cell_begin = begins_ptr[cell_idx];
      const int n = sizes_ptr[cell_idx];
      ...
      for (int inner_idx = 0; inner_idx < n; ++inner_idx) {
        const int64_t embedding_pos = cell_begin + inner_idx;  // Global embedding position
        const int32_t pid = codes_ptr[embedding_pos];
        const uint8_t *residual = residuals_ptr + (
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("parallel_decompress_centroids_with_winners_2_cpp", &parallel_decompress_centroids_with_winners<2>, 
        "Decompress Centroid Embeddings with Winner Tracking (nbits=2)");
  m.def("parallel_decompress_centroids_with_winners_4_cpp", &parallel_decompress_centroids_with_winners<4>, 
